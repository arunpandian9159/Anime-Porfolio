...
const EducationCard = memo(({ edu, index }) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`opacity-0 ${edu.isMain ? "bg-linear-to-br from-punch-red/10 to-oxford-navy/50 border-punch-red" : "bg-oxford-navy-dark/50 border-frosted-blue/15"} border rounded-2xl p-7 text-center transition-all hover:-translate-y-1 hover:border-punch-red`}
    >
      ...
      <span className="inline-block px-4 py-1 bg-frosted-blue/10 rounded-full text-sm text-frosted-blue mb-4">
        {edu.year}
      </span>
      ...
      <span className="inline-block px-5 py-2 bg-punch-red text-honeydew rounded-full font-semibold">
        {edu.grade}
      </span>
    </div>
  );
